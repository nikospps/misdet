...
def app():
    st.header(f":rainbow[Prediction of NLP Tasks by Given Text]")
    ...
    st.markdown(f"### :gray[Article Text]")
    title = st.text_area("Insert some title here", value="This is a text tile")
    text = st.text_area("Insert some text context here", value="This is a text context")
    len(text[:2000])
    len(text)
